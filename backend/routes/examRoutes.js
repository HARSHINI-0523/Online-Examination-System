...
router.post("/create", authMiddleware, async (req, res) => {
  try {
    ...
    if (examType === "online-test") {
      examLink = `https://online-examination-system-zeta.vercel.app/dashboard/exam/${uuidv4()}`;
    }
...
router.get("/get-exam/:examId", async (req, res) => {
  try {
    ...
    const exam = await Exam.findOne({ examLink: `https://online-examination-system-zeta.vercel.app/dashboard/exam/${req.params.examId }`});
